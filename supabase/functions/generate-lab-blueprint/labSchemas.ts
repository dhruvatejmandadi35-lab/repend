// Mirrors src/lib/labSchemas.ts. Kept duplicated because edge functions run in
// Deno and cannot import from the React src/ tree. Update both files together.
...
const blockSchema = z.object({
  type: z.enum([
    ...
    "output_display", "table", "chart", "insight", "diagram", "step_task",
  ]),
}).passthrough();
...
export const codeDebuggerSchema = z.object({
...
export const highlightSelectSchema = z.object({
...
export const debateBuilderSchema = z.object({
...
export const budgetAllocatorSchema = z.object({
...
const cohesiveActivitySchema = z.object({
  id: z.string(),
...
export const cohesiveSchema = z.object({
  ...
  verdict_tiers: z.array(z.object({
...
export const artifactSchema = z.object({
  ...
  if (d.reflection_question) {
    if (!d.reflection_options || d.reflection_options.length < 2) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["reflection_options"], message: "must have at least 2 entries when reflection_question is set" });
    }
    if (!d.reflection_correct) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["reflection_correct"], message: "required when reflection_question is set" });
    } else if (d.reflection_options && !d.reflection_options.includes(d.reflection_correct)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["reflection_correct"], message: "must match one of reflection_options" });
    }
  }
});

export const LAB_SCHEMAS: Record<string, z.ZodTypeAny> = {
...
export type LabValidationResult =
  ...
  | LabValidationFailure;

export function validateLabData(labType: string, data: unknown): LabValidationResult {
  const schema = LAB_SCHEMAS[labType];
  if (!schema) {
    return {
      ok: false,
      labType,
